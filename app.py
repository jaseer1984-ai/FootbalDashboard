...
# Modern UI with cards, light theme, tabs, and enhanced filters
...
def inject_beautiful_css():
    ...
    <style>
      ...
      .stButton>button, .stDownloadButton>button{
        ...
        border: 0 !important; border-radius: 12px !important; padding: .75rem 1.5rem !important;
        font-weight: 600 !important; font-size: .875rem !important; letter-spacing:.05em; text-transform: uppercase;
        box-shadow: 0 4px 12px rgba(14,165,233,.2); transition: .3s ease !important;
      }
      .stButton>button:hover, .stDownloadButton>button:hover{
        background: linear-gradient(135deg, #0284c7, #0369a1) !important;
        transform: translateY(-2px); box-shadow: 0 8px 20px rgba(14,165,233,.3);
      }
      ...
      /* Info card */
      .info-card{ background: linear-gradient(135deg,#fef3c7,#fbbf24); border:1px solid #f59e0b; border-radius:12px;
                  padding:1rem; color:#92400e; text-align:center; font-weight:500; box-shadow:0 2px 4px rgba(245,158,11,.1); }
...
def display_beautiful_metrics(df: pd.DataFrame) -> None:
    ...
    col1.metric("⚽ TOTAL GOALS", f"{total:,}")
    col2.metric("👥 PLAYERS", f"{players:,}")
    col3.metric("🏆 TEAMS", f"{teams:,}")
    col4.metric("📊 AVG GOALS/PLAYER", f"{avg_goals}")

def create_filter_panel(full_df: pd.DataFrame):
    st.markdown("""
    <div class="filter-panel"><div class="filter-title">🔍 Smart Filters & Search</div></div>
    """, unsafe_allow_html=True)
    ...
    with col2:
        ...
        team_sel = [t[2:] for t in team_sel]
...
def _integer_axis(values_series: pd.Series) -> alt.Axis:
    """Return an Altair axis that only shows integer ticks."""
    try:
        max_val = int(values_series.max()) if len(values_series) else 1
    except Exception:
        max_val = 1
    ticks = list(range(0, max_val + 1)) if max_val <= 50 else None
    return alt.Axis(format="d", tickMinStep=1, values=ticks)

def create_altair_bar_chart(df: pd.DataFrame, x_col: str, y_col: str, title: str, color_col=None):
    base = alt.Chart(df)
    if color_col:
        chart = base.mark_bar(cornerRadiusTopLeft=4, cornerRadiusTopRight=4, stroke='white', strokeWidth=1).encode(
            x=alt.X(f'{x_col}:Q', title=x_col.replace('_',' ').title(), axis=_integer_axis(df[x_col])),
        ...
        chart = base.mark_bar(cornerRadiusTopLeft=4, cornerRadiusTopRight=4, color='#0ea5e9', stroke='white', strokeWidth=1).encode(
            x=alt.X(f'{x_col}:Q', title=x_col.replace('_',' ').title(), axis=_integer_axis(df[x_col])),
            y=alt.Y(f'{y_col}:N', sort='-x', title=y_col.replace('_',' ').title()),
            tooltip=[y_col, x_col]
        )
    return chart.properties(title=alt.TitleParams(text=title, fontSize=16, anchor='start', color='#1e293b'), height=400)
...
def create_pie_chart(df: pd.DataFrame) -> alt.Chart:
    ...
    text = alt.Chart(agg).mark_text(fontSize=12, fontWeight='bold', color='white').encode(
...
def create_advanced_analytics_charts(df: pd.DataFrame):
    ...
    scatter = alt.Chart(team_stats).mark_circle(size=100, stroke='white', strokeWidth=2).encode(
        x=alt.X('Player_Count:Q', title='Number of Players', axis=_integer_axis(team_stats['Player_Count'])),
        y=alt.Y('Total_Goals:Q', title='Total Goals', axis=_integer_axis(team_stats['Total_Goals'])),
...
def main():
    ...
    if div_sel != "All Divisions" or team_sel or player_query.strip() or (current_players and 'players_pick' in locals() and players_pick):
        with st.expander("🔍 Active Filters", expanded=False):
            ...
            if team_sel: st.write(f"**Teams:** {', '.join(team_sel)}")
            if player_query.strip(): st.write(f"**Search:** {player_query}")
    ...
    with t_overview:
        ...
        if max_rows == 0:
            ...
            st.dataframe(top_df, use_container_width=True, hide_index=True,
                         column_config={"Player": st.column_config.TextColumn("👤 Player", width="medium"),
                                        "Team": st.column_config.TextColumn("⚽ Team", width="medium"),
                                        "Goals": st.column_config.NumberColumn("⚽ Goals", format="%d", width="small")})
            if max_rows > 1:
        ...
        if div_sel == "All Divisions" and not full_df.empty:
            ...
            with col2:
                ...
                st.dataframe(div_summary, use_container_width=True, hide_index=True,
                             column_config={"Division": st.column_config.TextColumn("🏅 Division"),
    ...
    with t_teams:
        ...
        if filtered.empty:
            ...
            top_by_team = top_by_team.sort_values(["Team","Goals"], ascending=[True,False]).groupby("Team").head(1).rename(columns={"Player":"Top Scorer","Goals":"Top Scorer Goals"})
            teams_list = (team_div.merge(team_summary, on="Team").merge(top_by_team, on="Team").sort_values(["Total_Goals","Team"], ascending=[False,True]))
            ...
            st.dataframe(teams_list, use_container_width=True, hide_index=True,
                         column_config={"Division": st.column_config.TextColumn("🏅 Division", width="small"),
                                        "Team": st.column_config.TextColumn("⚽ Team", width="medium"),
    ...
    with t_players:
        ...
        if filtered.empty:
            ...
            st.dataframe(players_list, use_container_width=True, hide_index=True,
            ...
            with col1: st.metric("🌟 Highest Scorer", f"{players_list.iloc[0]['Player']}", f"{players_list.iloc[0]['Goals']} goals")
            with col2: st.metric("📊 Average Goals", f"{players_list['Goals'].mean():.1f}")
            with col3: st.metric("📈 Median Goals", f"{players_list['Goals'].median()}")
    ...
    with t_downloads:
        ...
        with col2:
            ...
            st.download_button("📦 Complete Report Package (ZIP)", data=zip_bytes,
