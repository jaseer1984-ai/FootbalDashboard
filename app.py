...
def create_sample_tournament_data() -> pd.DataFrame:
    """Create sample tournament data for demonstration - matches current Excel structure"""
    return pd.DataFrame([
        {"Division": "B Division", "Team": "Bluestar B", "Player": "Alan Solaman Kaithavalappill", "Goals": 2, "Yellow_Cards": 0, "Red_Cards": 0, "Appearances": 0},
        {"Division": "B Division", "Team": "Blasters B", "Player": "Mohammed Niyas Melayil Abdul Azeez", "Goals": 2, "Yellow_Cards": 0, "Red_Cards": 0, "Appearances": 0},
        {"Division": "A Division", "Team": "Zabin A", "Player": "Abdul Raheem Kallan", "Goals": 2, "Yellow_Cards": 0, "Red_Cards": 0, "Appearances": 0},
        {"Division": "A Division", "Team": "ACC", "Player": "Mohammed Rafeeque Cherukunnan", "Goals": 0, "Yellow_Cards": 0, "Red_Cards": 0, "Appearances": 0},
        {"Division": "A Division", "Team": "Real Kerala", "Player": "Mohammed Ajsal", "Goals": 1, "Yellow_Cards": 0, "Red_Cards": 0, "Appearances": 0},
        {"Division": "B Division", "Team": "ACC A Division", "Player": "Salman Faris Karadan", "Goals": 1, "Yellow_Cards": 0, "Red_Cards": 0, "Appearances": 0},
        {"Division": "B Division", "Team": "Real Kerala", "Player": "Sanooj Mundakkaparamban", "Goals": 0, "Yellow_Cards": 0, "Red_Cards": 0, "Appearances": 0},
        {"Division": "A Division", "Team": "Age", "Player": "Muhamed Ashik Pulparamban", "Goals": 0, "Yellow_Cards": 0, "Red_Cards": 0, "Appearances": 0},
    ])
...
def create_division_comparison(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "Yellow_Cards" in df.columns:
        yellow_sum = df.groupby("Division")["Yellow_Cards"].sum().reset_index()
        division_stats = division_stats.merge(yellow_sum.rename(columns={"Yellow_Cards": "Total_Yellow_Cards"}), on="Division")
    
    if "Red_Cards" in df.columns:
        red_sum = df.groupby("Division")["Red_Cards"].sum().reset_index()
        division_stats = division_stats.merge(red_sum.rename(columns={"Red_Cards": "Total_Red_Cards"}), on="Division")
    
    if "Appearances" in df.columns:
        app_sum = df.groupby("Division")["Appearances"].sum().reset_index()
        division_stats = division_stats.merge(app_sum.rename(columns={"Appearances": "Total_Appearances"}), on="Division")
