...
# Players Card View + CARDS v2 + POINT TABLE + PDF Report
...
# -------- PDF (add 'reportlab' to requirements.txt) --------
...
def inject_advanced_css():
    ...
    <style>
        ...
        /* ---- TABLE ALIGNMENT (applies to every st.dataframe / st.data_editor) ---- */
        /* Center all column headers */
        [data-testid="stDataFrame"] div[role="columnheader"] {
            justify-content: center !important;
            text-align: center !important;
        }
        /* Center all grid cells (numbers like Goals/Yellow/Red/Match/Pts will now be centered) */
        [data-testid="stDataFrame"] div[role="gridcell"] {
            justify-content: center !important;
            text-align: center !important;
        }

        /* ---- Narrow box for Teams Summary ---- */
        .teams-summary-narrow{
            max-width: 980px;               /* adjust as you like (e.g., 900–1100px) */
        ...
        .pcard .action{ margin-top:.5rem; margin-right:.4rem; font-size:.78rem; color:#1f2937; background:#f8fafc; border:1px solid #e5e7eb; border-radius:999px; padding:.18rem .55rem; display:inline-block; }
...
def find_division_columns(raw_df: pd.DataFrame):
    ...
    for row_idx in range(min(2, len(raw_df))):
        ...
        for col_idx, cell in row.items():
            if "b division" in cell and b_col is None: b_col = col_idx
            elif "a division" in cell and a_col is None: a_col = col_idx
...
def process_cards_sheet(sheet_df: pd.DataFrame) -> pd.DataFrame:
    ...
    last = (
        events.sort_values(keys+["Match"])
              .groupby(keys, as_index=False).tail(1)[keys+["Match","Action"]]
              .rename(columns={"Match":"Last Match","Action":"Last Action"})
    )
    evsum = (
        events.sort_values(keys+["Match"]).groupby(keys)["Event"]
              .apply(lambda s: " • ".join([str(x) for x in s if _norm_text(x)])).reset_index()
...
def _norm_div_key(s: str) -> str:
    ...
    if "a" in s2 and "division" in s2: return "adivision"
    if "b" in s2 and "division" in s2: return "bdivision"
    return re.sub(r"[^a-z0-9]", "", s2)

def canonicalize_names(df: pd.DataFrame) -> pd.DataFrame:
    out = df.copy()
    def canonize_column(col: str):
        if col not in out.columns: return
        tmp = out.dropna(subset=[col]).copy()
        if tmp.empty: return
        tmp["k"] = tmp[col].apply(_norm_key)
        pref = (tmp.groupby(["k", col]).size().reset_index(name="n")
                   .sort_values(["k","n",col], ascending=[True,False,True]).drop_duplicates("k"))
    ...
    if "Team" in out.columns:   out["TeamKey"]   = out["Team"].apply(_norm_key)
    if "Player" in out.columns: out["PlayerKey"] = out["Player"].apply(_norm_key)
    if "Division" in out.columns: out["DivKey"]  = out["Division"].apply(_norm_div_key)
    return out
...
def merge_goals_cards(goals: pd.DataFrame, cards: pd.DataFrame) -> pd.DataFrame:
    ...
    merged = (base.merge(cards, on=["Player","Team","Division"], how="outer", suffixes=("","_cards"))
    ...
    for c in ["Division","Team","Player","Last Action","Card Events"]:
        if c in merged.columns: merged[c] = merged[c].apply(_norm_text)
    ...
    return canonicalize_names(merged)
...
def create_division_comparison(df: pd.DataFrame) -> pd.DataFrame:
    ...
    division_stats = (df.groupby("Division")
                        .agg(Goals_sum=("Goals","sum"), Goals_mean=("Goals","mean"),
                             ...
                             Players=("Player","nunique")).round(2).reset_index()
...
def create_horizontal_bar_chart(df: pd.DataFrame, x_col: str, y_col: str, title: str, color_scheme: str = "blues") -> alt.Chart:
    ...
    return (
        ...
        .encode(
            x=alt.X(f"{x_col}:Q", title="Goals",
                    axis=alt.Axis(format="d", tickMinStep=1, values=tick_values, gridOpacity=.3),
        ...
        .properties(height=max(300, min(600, len(df)*25)),
...
def create_advanced_scatter_plot(df: pd.DataFrame):
    ...
    if PLOTLY_AVAILABLE:
        ...
        fig.update_traces(marker=dict(sizemode="diameter", sizemin=8, sizemax=30,
                                      line=dict(width=2, color="white"), opacity=.85))
        ...
        return fig
    return (
        alt.Chart(team_stats).mark_circle(size=100, opacity=.85)
...
def render_player_cards(df: pd.DataFrame):
    ...
    players = (work.groupby(["Player","Team","Division"]).agg(**agg_map).reset_index().fillna(0))
    ...
    for _, row in players.iterrows():
        ...
        if "Appearances" in players.columns:
            val = int(row["Appearances"]); pct = f"{(val/max(max_app,1))*100:.1f}%"
            html.append('<div class="row"><div class="label">👕 Appearances</div>')
            html.append(f'<div class="dotbar"><span style="--pct:{pct}"></span></div><div class="num">{val}</div></div>')

        if "YellowCards" in players.columns:
            val = int(row["YellowCards"]); pct = f"{(val/max(max_yel,1))*100:.1f}%"
            html.append('<div class="row"><div class="label">🟨 Yellow Cards</div>')
            html.append(f'<div class="dotbar"><span style="--pct:{pct}"></span></div><div class="num">{val}</div></div>')

        if "RedCards" in players.columns:
            val = int(row["RedCards"]); pct = f"{(val/max(max_red,1))*100:.1f}%"
            html.append('<div class="row"><div class="label">🟥 Red Cards</div>')
            html.append(f'<div class="dotbar"><span style="--pct:{pct}"></span></div><div class="num">{val}</div></div>')

        chips = []
        if "CardEvents" in players.columns and pd.notna(row.get("CardEvents", None)):
            for piece in str(row["CardEvents"]).split(" • "):
                if piece.strip(): chips.append(f'<span class="action">{piece.strip()}</span>')
        ...
        html.append('<div style="margin-top:.5rem">' + ("".join(chips) if chips else '<span class="pill">No awards</span>') + "</div>")
        html.append('</div>')
    html.append('</div>')
...
# ====================== PDF REPORT ================================
def generate_pdf_report(full_df: pd.DataFrame, filtered_df: pd.DataFrame, teams_summary: pd.DataFrame, stats: dict) -> bytes:
    buf = BytesIO()
    doc = SimpleDocTemplate(buf, pagesize=A4, leftMargin=1.6*cm, rightMargin=1.6*cm, topMargin=1.6*cm, bottomMargin=1.6*cm)
    styles = getSampleStyleSheet()
    styles.add(ParagraphStyle(name="TitleBig", parent=styles["Title"], alignment=1, fontName="Helvetica-Bold", fontSize=20, spaceAfter=12))
    styles.add(ParagraphStyle(name="H2", parent=styles["Heading2"], fontSize=14, spaceAfter=10))
    styles.add(ParagraphStyle(name="Center", parent=styles["BodyText"], alignment=1))
    ...
    story += [
        Spacer(1, 3*cm),
        Paragraph("ABEER BLUESTAR SOCCER FEST 2K25", styles["TitleBig"]),
        Paragraph("Detailed Tournament Report", styles["Center"]),
        Spacer(1, 0.5*cm),
        Paragraph(datetime.now().strftime("%d %b %Y, %H:%M"), styles["Center"]),
        Spacer(1, 2*cm),
        Paragraph("This report includes overall statistics, top scorers, team summaries, and standings points merged from the official sheets.", styles["BodyText"]),
        Spacer(1, 1.5*cm),
        Paragraph("Prepared automatically by the Streamlit dashboard.", styles["BodyText"]),
        Spacer(1, 2*cm),
    ]

    # Summary metrics table
    story += [Paragraph("Overview", styles["H2"])]
    smry = [
        ["Total Goals", stats["total_goals"]],
        ["Players", stats["total_players"]],
        ["Teams", stats["total_teams"]],
        ["Divisions", stats["divisions"]],
        ["Yellow Cards", stats["total_yellow"]],
        ["Red Cards", stats["total_red"]],
        ["Avg Goals/Team", stats["avg_goals_per_team"]],
    ]
    t = Table(smry, colWidths=[7*cm, 7*cm])
    ...
    # Top scorers (top 10)
    story += [Paragraph("Top Scorers (Top 10)", styles["H2"])]
    if not filtered_df.empty:
        tops = (filtered_df.groupby(["Player","Team"])["Goals"].sum().reset_index()
                .sort_values(["Goals","Player"], ascending=[False,True]).head(10))
        data = [["Player", "Team", "Goals"]] + tops.values.tolist()
        t2 = Table(data, colWidths=[6*cm, 6*cm, 2*cm])
        ...
        story += [Paragraph("No player data available for current filters.", styles["BodyText"]), Spacer(1, 1*cm)]

    # Teams summary (top 12 by goals)
    story += [Paragraph("Teams Summary", styles["H2"])]
    if teams_summary is not None and not teams_summary.empty:
        ts = teams_summary.sort_values("Total_Goals", ascending=False).head(12)
        columns = ["Team","Division","Players","Total_Goals","Pts"]
        data = [ ["Team","Division","Players","Goals","Pts"] ] + ts[columns].values.tolist()
        t3 = Table(data, colWidths=[6*cm, 3*cm, 2*cm, 2*cm, 2*cm])
        ...
        story += [Paragraph("No team summary available.", styles["BodyText"])]
...
def create_comprehensive_zip_report(full_df: pd.DataFrame, filtered_df: pd.DataFrame) -> bytes:
    ...
    with zipfile.ZipFile(zip_buffer, "w", zipfile.ZIP_DEFLATED) as z:
        ...
        if not filtered_df.empty:
            ...
            if not div_cmp.empty: z.writestr("05_division_comparison.csv", div_cmp.to_csv(index=False))
...
def create_download_section(full_df: pd.DataFrame, filtered_df: pd.DataFrame, teams_summary: pd.DataFrame, stats: dict):
    ...
    with col1:
        ...
        if not full_df.empty:
            st.download_button("⬇️ FULL Dataset (CSV)", data=full_df.to_csv(index=False),
                               file_name=f"tournament_full_{datetime.now():%Y%m%d_%H%M}.csv", mime="text/csv")
        if not filtered_df.empty:
            st.download_button("⬇️ FILTERED Dataset (CSV)", data=filtered_df.to_csv(index=False),
                               file_name=f"tournament_filtered_{datetime.now():%Y%m%d_%H%M}.csv", mime="text/csv")

    with col2:
        st.subheader("📝 PDF Report")
        if not filtered_df.empty:
            pdf_bytes = generate_pdf_report(full_df, filtered_df, teams_summary, stats)
            st.download_button("⬇️ Detailed PDF Report", data=pdf_bytes,
                               file_name=f"tournament_report_{datetime.now():%Y%m%d_%H%M}.pdf", mime="application/pdf")

    with col3:
        st.subheader("📦 Complete ZIP")
...
def _coerce_first_row_as_header(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for c in header:
        ...
        seen[c] = seen.get(c,0)+1; cols.append(k)
    df2.columns = cols
...
def normalize_point_table(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "Team" in df.columns: df["Team"] = df["Team"].astype(str).str.strip()
    ...
    if sort_cols: df = df.sort_values(sort_cols, ascending=[False]+[False]*(len(sort_cols)-1), kind="mergesort")
...
def _pick_sheet_name(all_names: list[str], target: str) -> str | None:
    ...
    for nm in all_names:
        if _norm_sheetname(nm) == want: return nm
    for nm in all_names:
        if _norm_sheetname(target) in _norm_sheetname(nm): return nm
    return None
...
def display_point_table(df: pd.DataFrame):
    if df is None or df.empty:
        st.info("No data found for this group yet."); return
    show = df.copy()
...
<div class="app-title">
  ...
  <span class="title">ABEER BLUESTAR SOCCER FEST 2K25</span>
    ...
    with st.sidebar:
        ...
        selected_players = st.multiselect("Type to search and select players", options=player_names, default=[],
        ...
        card_filter = st.selectbox("Show players with…", ["All","Yellow only","Red only"], index=0, key="cards_filter")
    ...
    # Build PTS lookup from Google Sheets
    def build_pts_lookup() -> pd.DataFrame:
        sections = fetch_point_tables(POINT_TABLE_URL)
        blocks = []
        for label, dfpts in sections.items():
            if dfpts is None or dfpts.empty or "Team" not in dfpts.columns: continue
            if "Pts" not in dfpts.columns: continue
        ...
        if not blocks: return pd.DataFrame(columns=["TeamKey","DivKey","Pts"])
        pts_all = pd.concat(blocks, ignore_index=True)
        return pts_all.groupby(["TeamKey","DivKey"], as_index=False)["Pts"].max()
    ...
    with tab3:
        ...
        if tournament_data.empty:
            ...
            teams_summary = (tournament_data.groupby(["Team","Division"])
            ...
            # ---- Narrow width table ----
            st.markdown('<div class="teams-summary-narrow">', unsafe_allow_html=True)
            st.dataframe(
    ...
    with tab6:
        ...
        try:
            ...
            with sub1: display_point_table(sections.get("B-Division Group A"))
            with sub2: display_point_table(sections.get("B-Division Group B"))
            with sub3: display_point_table(sections.get("A-Division"))
        except Exception as e:
            st.error(f"Couldn't load the Point Table workbook. {e}")

    with tab7:
        # provide PDF here too (needs teams_summary from Teams tab)
        # If Teams tab hasn't run yet, recompute a minimal teams_summary:
        ts_for_pdf = (tournament_data.groupby(["Team","Division"])
                      .agg(Players=("Player","nunique"), Total_Goals=("Goals","sum")).reset_index())
        ts_for_pdf["TeamKey"] = ts_for_pdf["Team"].apply(_norm_key)
        ts_for_pdf["DivKey"]  = ts_for_pdf["Division"].apply(_norm_div_key)
        if not pts_lookup_df.empty:
            ts_for_pdf = ts_for_pdf.merge(pts_lookup_df, on=["TeamKey","DivKey"], how="left")
        else:
            ts_for_pdf["Pts"] = 0
        ts_for_pdf["Pts"] = ts_for_pdf["Pts"].fillna(0).astype(int)
        ts_for_pdf = ts_for_pdf.drop(columns=["TeamKey","DivKey"])

        create_download_section(full_tournament_data, tournament_data, ts_for_pdf, current_stats)
