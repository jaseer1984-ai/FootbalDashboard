...
# - Robust XLSX parsing with safe fallback (no openpyxl required)
...
def inject_css():
    ...
    <style>
      ...
      /* Sidebar look */
      section[data-testid="stSidebar"]{background:linear-gradient(180deg,#f8fafc 0%,#ffffff 100%) !important;border-right:1px solid #e2e8f0;}
      section[data-testid="stSidebar"] h2, section[data-testid="stSidebar"] h3{color:#1e293b !important;font-weight:600;}

      /* Buttons */
      .stButton>button,.stDownloadButton>button{
        background:linear-gradient(135deg,#0ea5e9,#0284c7) !important;color:#fff !important;border:0 !important;
        border-radius:12px !important;padding:.65rem 1.1rem !important;font-weight:600 !important;font-size:.84rem !important;
      ...
      .stTabs [data-baseweb="tab"]{background:transparent;border-radius:8px;color:#475569;font-weight:600;border:none;transition:.2s;padding:.5rem .9rem;}
      .stTabs [data-baseweb="tab"]:hover{ background:#eef6ff; color:#0f172a; }
      .stTabs [data-baseweb="tab"][aria-selected="true"]{ background:linear-gradient(135deg,#0ea5e9,#0284c7) !important;color:#fff !important; box-shadow:0 2px 8px rgba(14,165,233,.3); }

      /* Tables */
      .stDataFrame{background:#fff;border-radius:12px;overflow:hidden;border:1px solid #e2e8f0;box-shadow:0 2px 4px rgba(0,0,0,.05);}

      /* Section headers */
      .section-header{font-size:1.5rem;font-weight:700;color:#1e293b;margin:2rem 0 1rem;padding-bottom:.5rem;border-bottom:2px solid #e2e8f0;letter-spacing:.01em;}

      /* Info card */
      .info-card{ background:linear-gradient(135deg,#fef3c7,#fbbf24);border:1px solid #f59e0b;border-radius:12px;padding:1rem;color:#92400e; text-align:center;font-weight:600;box-shadow:0 2px 4px rgba(245,158,11,.1); }
      ...
      /* Inputs */
      .stSelectbox label,.stMultiSelect label,.stTextInput label{color:#1e293b !important;font-weight:600 !important;}
      ...
      [data-testid="metric-container"] [data-testid="stMetricLabel"]{color:#64748b !important;font-weight:600 !important;text-transform:uppercase;font-size:.75rem;letter-spacing:.05em;}
...
def _parse_xlsx_without_openpyxl(file_bytes: bytes) -> pd.DataFrame:
    """Very simple first-sheet reader using the OOXML structure (sharedStrings + first sheetN.xml)."""
    ...
    with zipfile.ZipFile(BytesIO(file_bytes)) as z:
        ...
        # Pick the first sheet xml we find (sheet1.xml, sheet2.xml, ...)
...
def _read_excel_raw(file_like_or_bytes) -> pd.DataFrame:
    """Try pandas -> fallback to our OOXML parser on *any* failure."""
    if isinstance(file_like_or_bytes, (str, Path)):
        with open(Path(file_like_or_bytes), "rb") as fh:
            b = fh.read()
        src = BytesIO(b)
    ...
    try:
        # header=None to keep a raw grid; we’ll reshape later
        return pd.read_excel(src, header=None)
    except Exception:
        return _parse_xlsx_without_openpyxl(b)

# ---------------------- Parser for the dual-table sheet -----------------------
def _find_block_start_indices(raw: pd.DataFrame):
    """Find the starting columns of 'B Division' and 'A Division' headers in the first two rows."""
    for row_idx in (0, 1):
        if row_idx >= len(raw):
            continue
...
def load_and_prepare_data_from_bytes(xlsx_bytes: bytes) -> pd.DataFrame:
    ...
    # Try to locate blocks; if not found, assume two 3-col blocks side-by-side
    ...
    # Header row is usually the row after the division header
...
def reports_zip(full_df: pd.DataFrame, filtered_df: pd.DataFrame) -> bytes:
    ...
    top_scorers = (filtered_df.groupby(["Player","Team"], as_index=False)["Goals"].sum()
                   .sort_values(["Goals","Player"], ascending=[False,True])[["Player","Team","Goals"]])
    buf = BytesIO()
...
def main():
    ...
    if player_q.strip():
        ...
        if toks:
            # robust boolean mask (avoid Series|bool pitfalls)
            mask = pd.Series(False, index=filtered.index)
            pl = filtered["Player"].astype(str).str.lower()
            for t in toks:
                mask = mask | pl.str.contains(t, na=False)
