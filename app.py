# app.py — ABEER BLUESTAR SOCCER FEST 2K25 (heading fix + KPI cards)
...
def main():
    ...
    full_df=data.copy()  # keep unfiltered copy for FULL download
    ...
    player_goals_total=(filtered.groupby("Player", as_index=False)["Goals"].sum()
                        .sort_values("Goals", ascending=False))
    n=int(player_goals_total.shape[0])
    if n==0:
        st.info("No player data to display for the current filters.")
    elif n==1:
        st.caption("Only one player found — showing that player.")
        single=player_goals_total.head(1).reset_index(drop=True)
        st.dataframe(single, use_container_width=True, hide_index=True,
                     column_config={"Goals": st.column_config.NumberColumn("Goals", format="%d")})
        st.altair_chart(bar_chart(single, "Player", "Goals", "Top 1 Player by Goals"), use_container_width=True)
    else:
        default_top=min(10,n)
        top_n=st.sidebar.slider("Top N players", 1, n, default_top, key=f"topn_{n}")
